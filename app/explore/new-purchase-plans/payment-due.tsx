...
const PaymentDue = () => {
  ...
  const addRecieptMast = async (receipt: number) => {
    ...
    const payload = {
      ...
      goldRate: 0,
      cardNo: String(params?.CARDNO), // ✅ Convert to string
  ...
  const addRecieptPayment = async (receipt: number) => {
    const userName = await AsyncStorage.getItem("userName");
    const tablePayloads = [
  ...
  const addMemberDetails = async (receipt: number) => {
    const userName = await AsyncStorage.getItem("userName");
  ...
  const addMember = async (receipt: number) => {
    const userName = await AsyncStorage.getItem("userName");
    const payload = {
      schemeGroup: params?.SchemeGroup || "string",
      schemeName: params?.SchemeName || "string",
      ...
      cardNo: String(params?.CARDNO) || "string",
      schemeAmount: params?.SchemeAmount || 0,
      schemeDuration: params?.SchemeDuration || 0,
      bonusAmount: params?.BonusAmount || 0,
      schemeValue: params?.SchemeValue || 0,
  ...
  const handleCreateApi = async () => {
    try {
      ...
      await addRecieptMast(receipt);
      await addMemberDetails(receipt);
      await addRecieptPayment(receipt);
      await addMember(receipt);
